...
def encodeDataInImage(image, data):
    ...
    encodedPixels = [(r + int(binary[index * 4 + 0]), g + int(binary[index * 4 + 1]), b + int(binary[index * 4 + 2]),
                      t + int(binary[index * 4 + 3])) if index * 4 < len(binary) else (r, g, b, t) for index, (r, g, b, t) in enumerate(list(evenImage.getdata()))]  # 将 binary 中的二进制字符串信息编码进像素里
...
if __name__ == "__main__":
    encodeDataInImage(Image.open("coffee.png").convert('RGBA'), '你好世界，Hello world!').save('encodeImage.png')
